package  
{
	...
	public class PlayState extends FlxState
	{
		...
		override public function update():void
		{
			...
			{
				if (actorArray[i] is Gem)
				{
					if (player.overlaps(actorArray[i]))
					{
						gemsCollected++;
						RemoveGemPermanently(actorArray[i] as Gem);
						actorArray[i] = null;
						gemsCounterTimer = gemsCounterTimerMax;
						gemsCounter.text = "Gems: " + gemsCollected;
						FlxG.play(Resources.coinSfx, 0.3);
					}
				}
				else if (actorArray[i] is BootPickup)
				{
					if (player.overlaps(actorArray[i]))
					{
		...
		public function LoadRoom(room:Room):void
		{
			...
			{
				if (room.backgroundMap == Resources.ReadMap(Resources.CentralBackground))
					background.loadMap(room.backgroundMap, room.backgroundTileset, 320, 240, FlxTilemap.OFF, 0, 0, 0);
				else
			...
			for each (var gem:Gem in currentRoom.retrievedGems)
			{
				foregroundCollidable.setTile(gem.tileX, gem.tileY, 0);
			}
			...
			for (i = 0; i < foregroundCollidable.width; i++)
			{
				...
				{
					if (foregroundCollidable.getTile(i, k) == gemTileIndex)
					{
						var gem:Gem = new Gem(i, k);
						actorArray.push(gem);
						add(gem);
						foregroundCollidable.setTile(i, k, 0);
					}
					else if (foregroundCollidable.getTile(i, k) == ffTileIndex)
					{
						if (currentRoom.forceFieldActive)
						{
							var forceField:ForceField = new ForceField(this, i, k, room.forceFieldGemsRequired);
							actorArray.push(forceField);
							forceFieldsGroup.add(forceField);
						}
						foregroundCollidable.setTile(i, k, 0);
					}
					else if (foregroundCollidable.getTile(i, k) == bootTileIndex)
					{
			...
			//Add force field gem text
			if (room.forceFieldActive)
			{
				//Get first active forcefield
				var activeField:ForceField = forceFieldsGroup.getFirstAlive() as ForceField;
				var ffText:FlxText = new FlxText(activeField.x, activeField.y - 32, FlxG.width, room.forceFieldGemsRequired.toString());
				ffText.setFormat(null, 8, 0xff0000);
				forceFieldTextGroup.add(ffText);
			}
		...
		public function OnPlayerDeath():void
		{
			...
			LoadRoom(world.GetRoom(player.saveRoomIndex));
